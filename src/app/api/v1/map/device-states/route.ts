...
function createDevice(index: number, timeBucket: number, now: Date) {
  ...
  return {
    ...
    device_type: {
      code: typeCode,
      name: TYPE_NAMES[typeIndex] ?? typeCode,
      icon_id: null,
      ui_panel: null,
    },
...
export function GET(request: NextRequest) {
  ...
  if (!bounds) {
    return NextResponse.json(
      { type: "about:blank", title: "Invalid bbox", status: 400, code: "INVALID_BBOX" },
      { status: 400 }
    );
  }
  ...
  const requestedTypes = new Set(
    (request.nextUrl.searchParams.get("device_types") ?? "").split(",").filter(Boolean)
  );
  ...
  const items = Array.from({ length: DEVICE_COUNT }, (_, index) =>
    createDevice(index, timeBucket, now)
  ).filter((device) => {
    ...
    const matchesStatus =
      requestedStatus === "all" || (requestedStatus === "online" ? device.online : !device.online);
    const matchesSearch =
      !search || `${device.code} ${device.name}`.toLocaleLowerCase("vi").includes(search);
