...
describe("device state store", () => {
  ...
  it("prevents void/static position movement while accepting newer metadata", () => {
    ...
    const second = makeDevice({ deviceId: first.deviceId, stateVersion: 2, positionVersion: 2, position: { type: "Point", coordinates: [107, 11] }, latestGpsStatus: "V", alertLevel: "high" });
    const store = useDeviceStateStore.getState(); store.beginRequest("q");
  ...
  it("keeps snapshot stale on error and refreshes timestamp on 304", () => {
    const store = useDeviceStateStore.getState(); store.beginRequest("q"); store.applySnapshot("q", makeSnapshot(1), null);
