...
function MapRuntime() {
  ...
  const devices = useMemo(() => [...visibleIds].flatMap((id) => { const device = devicesById.get(id); return device ? [device] : []; }), [devicesById, visibleIds]);
  useEffect(() => reconcileSelection(visibleIds), [reconcileSelection, visibleIds]);
  useEffect(() => { map.getContainer().focus({ preventScroll: true }); }, [map]);
  return <><ViewportBridge /><DeviceMarkerLayer devices={devices} /><MapStatusOverlay status={status} count={devices.length} unlocatedCount={unlocatedCount} lastSuccessAt={lastSuccessAt} error={error?.message ?? null} tooDense={tooDense} /></>;
}
...
export function DeviceMap() {
  ...
  return <main className="map-page"><header className="map-toolbar"><div><p className="eyebrow">CADPRO · LIVE MAP</p><h1>Bản đồ thiết bị</h1></div><DeviceMapFilters /></header><section className="map-shell" aria-label="Bản đồ thiết bị realtime"><MapContainer center={[10.7769, 106.7009]} zoom={13} minZoom={3} preferCanvas={false} keyboard className="leaflet-map"><TileLayer attribution={attribution} url={tileUrl} /><MapRuntime /></MapContainer></section><DeviceDetailPanel /></main>;
}
