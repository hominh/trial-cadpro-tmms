...
describe("map controls and detail", () => {
  ...
  it("supports partial code/name search", () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/Tìm mã hoặc tên/), {
      target: { value: "xe BuÝt" },
    });
  ...
  it("shows complete mobile details and explicit offline text", () => {
    ...
    const server = useDeviceStateStore.getState();
    server.beginRequest("q");
    server.applySnapshot("q", { ...makeSnapshot(1), items: [device] }, null);
