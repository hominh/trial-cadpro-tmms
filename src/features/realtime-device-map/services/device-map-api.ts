...
interface DeviceMapItemDto {
  ...
  position_source: "object_location" | "device_state_valid_gps";
...
interface SnapshotDto {
  ...
  items: DeviceMapItemDto[];
}

export type DeviceMapApiResult =
  | {
      readonly kind: "snapshot";
      readonly snapshot: DeviceMapSnapshot;
...
function assertSnapshotDto(value: unknown): asserts value is SnapshotDto {
  if (
    ...
    value.complete !== true ||
    value.offline_threshold_seconds !== 30
  ) {
...
function mapItem(item: DeviceMapItemDto): DeviceState {
  ...
  return {
    deviceId: asDeviceId(item.device_id),
    code: item.code,
    name: item.name,
    deviceType: {
      code: item.device_type.code,
      name: item.device_type.name,
      iconId: item.device_type.icon_id ?? null,
      uiPanel: item.device_type.ui_panel ?? null,
    },
    mobility: item.mobility,
    position: { type: "Point", coordinates: [longitude, latitude] },
    positionSource: item.position_source,
    positionObservedAt: item.position_observed_at,
    positionVersion: item.position_version,
    stateVersion: item.state_version,
    lastSeenAt: item.last_seen_at,
    online: item.online,
    speedKph: item.speed_kph ?? null,
    courseDeg: item.course_deg ?? null,
    isStatic: item.is_static,
    latestGpsStatus: item.latest_gps_status ?? null,
    alertLevel: item.alert_level,
    activePresetId: item.active_preset_id ?? null,
    presetSource: item.preset_source ?? null,
  };
}
...
function mapSnapshot(dto: SnapshotDto): DeviceMapSnapshot {
  return {
    ...
    query: {
      bbox: dto.query.bbox,
      deviceTypes: dto.query.device_types,
      status: dto.query.status,
      query: dto.query.q,
    },
    offlineThresholdSeconds: 30,
    returned: dto.returned,
    unlocatedCount: dto.unlocated_count,
    complete: true,
    pollAfterMs: dto.poll_after_ms,
    items: dto.items.map(mapItem),
  };
}

export function clearDeviceMapEtags(): void {
  etags.clear();
}

export async function fetchDeviceMapSnapshot(input: {
  ...
  try {
    const response = await apiClient.get<unknown>("/api/v1/map/device-states", {
      signal: input.signal,
      params: {
        bbox: serializeBbox(input.bounds),
        device_types: query.deviceTypes.length ? query.deviceTypes.join(",") : undefined,
        status: query.status,
    ...
    if (error instanceof Error && !(error instanceof AxiosError))
      throw <DeviceMapError>{ kind: "contract", message: error.message };
    if (error instanceof AxiosError && error.response?.status === 422) {
      const data = error.response.data as { matched?: number; max_items?: number };
      throw <DeviceMapError>{
        kind: "tooDense",
        message: "Viewport contains too many devices.",
        status: 422,
