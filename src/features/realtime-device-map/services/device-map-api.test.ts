...
describe("device map API", () => {
  ...
  afterEach(() => {
    deviceMapMockServer.resetHandlers();
    clearDeviceMapEtags();
  });
  afterAll(() => deviceMapMockServer.close());
  it("encodes bbox and maps 200 then ETag/304", async () => {
    const first = await fetchDeviceMapSnapshot({
      bounds,
      filters: DEFAULT_FILTERS,
      signal: new AbortController().signal,
    });
    expect(first.kind).toBe("snapshot");
    const second = await fetchDeviceMapSnapshot({
      bounds,
      filters: DEFAULT_FILTERS,
      signal: new AbortController().signal,
    });
  ...
  it("maps density errors", async () => {
    await expect(
      fetchDeviceMapSnapshot({
        bounds,
        filters: { ...DEFAULT_FILTERS, query: "too-dense" },
        signal: new AbortController().signal,
      })
    ).rejects.toMatchObject({ kind: "tooDense", status: 422 });
  });
  it("rejects malformed snapshots atomically", async () => {
    deviceMapMockServer.use(
      http.get("*/api/v1/map/device-states", () =>
        HttpResponse.json({ complete: false, items: [] })
      )
    );
    await expect(
      fetchDeviceMapSnapshot({
