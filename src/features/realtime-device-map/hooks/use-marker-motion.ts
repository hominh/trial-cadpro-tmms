...
export function useMarkerMotion() {
  ...
  const retarget = useCallback(
    (target: MotionTarget) => {
      ...
      const plan = createMotionPlan(
        target.fromConfirmed,
        target.to,
        target.fromObservedAtMs,
        target.toObservedAtMs
      );
      ...
      tracks.current.set(target.id, {
        marker: target.marker,
        from,
        to: target.to,
        startedAt: performance.now(),
        durationMs: plan.durationMs,
        fromCourse: target.fromCourse ?? 0,
        toCourse: target.toCourse ?? target.fromCourse ?? 0,
      });
  ...
  const remove = useCallback((id: DeviceId) => {
